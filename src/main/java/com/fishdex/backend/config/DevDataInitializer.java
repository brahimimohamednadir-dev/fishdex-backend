...
public class DevDataInitializer implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        ...
        Species brochet    = speciesRepository.findByCommonNameContainingIgnoreCase("Brochet", org.springframework.data.domain.Pageable.unpaged()).getContent().get(0);
        Species carpe      = speciesRepository.findByCommonNameContainingIgnoreCase("Carpe commune", org.springframework.data.domain.Pageable.unpaged()).getContent().get(0);
        Species sandre     = speciesRepository.findByCommonNameContainingIgnoreCase("Sandre", org.springframework.data.domain.Pageable.unpaged()).getContent().get(0);
        Species perche     = speciesRepository.findByCommonNameContainingIgnoreCase("Perche", org.springframework.data.domain.Pageable.unpaged()).getContent().get(0);
        Species silure     = speciesRepository.findByCommonNameContainingIgnoreCase("Silure", org.springframework.data.domain.Pageable.unpaged()).getContent().get(0);
        Species truite     = speciesRepository.findByCommonNameContainingIgnoreCase("Truite fario", org.springframework.data.domain.Pageable.unpaged()).getContent().get(0);
        Species barbeau    = speciesRepository.findByCommonNameContainingIgnoreCase("Barbeau", org.springframework.data.domain.Pageable.unpaged()).getContent().get(0);
        Species blackbass  = speciesRepository.findByCommonNameContainingIgnoreCase("Black-bass", org.springframework.data.domain.Pageable.unpaged()).getContent().get(0);
        Species chevaine   = speciesRepository.findByCommonNameContainingIgnoreCase("Chevaine", org.springframework.data.domain.Pageable.unpaged()).getContent().get(0);
        ...
        List<Capture> marcCaptures = captureRepository.saveAll(List.of(
            ...
            capture(marc, "Brochet",       brochet,  3.8, 78.0,  "Marne — Joinville",      48.8200,  2.4770, "Gros poisson blanc, leurre souple",   daysAgo(22)),
